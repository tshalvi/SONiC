...
| 0.3 | 2026-06-14 | Natanel Gerbi | Align §7.3 with the merged community PR [sonic-net/sonic-platform-common#682](https://github.com/sonic-net/sonic-platform-common/pull/682) ("Platform Support for CPO"): rework the vendor-extension surface to layer on top of the upstream `CpoBase` / `OeBase` / `ElsfpBase` hierarchy + `CpoApiFactory` (under `sonic_xcvr/cpo/`), and drop the alternate `xcvr_api_factory.create_xcvr_api(bank_id)` / `0x80` dispatch sketch. §7.4 (CDB pattern / aggregator overrides) is unchanged at the API-surface level and is now layered on top of the merged base classes. Add §7.1: replace the per-bank `Sfp(SfpBase)` transceiver object with a per-bank `CpoBase` -- sibling of `SfpBase` under a new common `XcvrBase` parent (see [tshalvi/sonic-platform-common#4](https://github.com/tshalvi/sonic-platform-common/pull/4)). |
<br>
...
### Added in Rev 0.3:
* Per-bank transceiver object (§7.1): the per-bank object exposed to chassis / `xcvrd` for a CPO module is a `CpoBase` instead of an `Sfp(SfpBase)` -- both now sit under a new common `XcvrBase` parent (see [tshalvi/sonic-platform-common#4](https://github.com/tshalvi/sonic-platform-common/pull/4)).
